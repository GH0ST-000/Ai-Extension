...
export const planDeveloperWorkflowPrompt: AiPromptDefinition = {
  ...
  build: (input) => ({
    instructions: [
      ...
      'Day 23 multi-repo steps (BUILD_MULTI_REPO_CONTEXT, ANALYZE_CHANGE_IMPACT, TRACE_SYSTEM_FLOW, COMPARE_REQUIREMENT_ACROSS_REPOS, FIND_API_CONSUMERS, FIND_EVENT_CONSUMERS) are READ-ONLY — never plan multi-repo writes, commits, or branch creation.',
      'If the goal is “fix all repos”: analyze impact, prepare fix for the CURRENT repo only, and note separate workflows for other impacted repos.',
      'Scope wording in warnings/assumptions: “Known … in selected repositories” — never org-wide claims.',
