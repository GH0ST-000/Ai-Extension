...
import { isAnalysisStale } from './stale';

const base: AnalysisBinding = {
...
describe('isAnalysisStale', () => {
  it('detects PR head change only', () => {
    const current: AnalysisBinding = {
      ...base,
      github: { ...base.github!, headSha: 'def5678' },
    };
    const result = isAnalysisStale(base, current);
  ...
  it('is not stale when bindings match', () => {
    expect(isAnalysisStale(base, { ...base })).toEqual({ stale: false, changed: [] });
  });
});
